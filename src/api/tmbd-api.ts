...
import { Cast, Episode, Genre, Season, Trailer } from '../interfaces'
...
axiosClient.interceptors.request.use((config) => {
  return {
    ...
    params: {
      ...config.params,
      api_key: '275fabe764713e9d3281f6014999d0f3',
    },
  }
})

export const getTrendings = async (mediaType: MediaType): Promise<Films[]> => {
  try {
...
export const getInTheaters = async (): Promise<Films[]> => {
  try {
    const { data } = await axiosClient.get<
      any,
      AxiosResponse<{
